...
export function userRoutes(app: Hono<{ Bindings: Env }>) {
  ...
  app.post('/api/leads', async (c) => {
    const body = (await c.req.json()) as CreateLeadRequest;
    // Basic server-side validation
    if (!body.name?.trim() || !body.phone?.trim() || !body.address?.trim()) {
      return bad(c, 'name, phone, and address are required');
    }
    ...
    return ok(c, { id: lead.id });
  });
  // EXISTING DEMO ROUTES
  app.get('/api/test', (c) => c.json({ success: true, data: { name: 'Flowline API' }}));
